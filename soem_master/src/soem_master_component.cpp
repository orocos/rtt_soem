	...
	bool SoemMasterComponent::configureHook()
	{
		...
		if(ec_init(m_ifname.c_str()) > 0)
		{
			...
			if ( ec_config(true,&m_IOmap) > 0 )
			{
				...
				{
					log(Info) <<"Safe operational state reached for all slaves."<<endlog();
				}
				else
				{
		...
		for(int i=1;i<=ec_slavecount;i++){
			...
			if(driver){
				...
				log(Info)<<"Created driver for "<<ec_slave[i].name<<", with address "<<ec_slave[i].configadr<<endlog();
				...
				log(Warning)<<"Could not create driver for "<<ec_slave[i].name<<endlog();
			}
		}
		return true;
	}
