...
namespace soem_beckhoff_drivers{
    ...
    
  {
    ...
    m_service->addOperation("rawRead",&SoemEL3062::rawRead,this,RTT::OwnThread).doc("Read raw value of channel i").arg("i","channel nr");
    m_service->addOperation("read",&SoemEL3062::read,this,RTT::OwnThread).doc("Read value to channel i").arg("i","channel nr");
    m_service->addOperation("Over_Range",&SoemEL3062::isOverrange,this,RTT::OwnThread).doc("For the channel i : 1 = overrange ; 0 = no overrange ").arg("i","channel nr");
    m_service->addOperation("Under_Range",&SoemEL3062::isUnderrange,this,RTT::OwnThread).doc("For the channel i : 1 = Underrange ; 0 = no Underrange ").arg("i","channel nr");
    m_service->addOperation("Comp_val_to_lim",&SoemEL3062::CompareV_to_Lim,this,RTT::OwnThread).doc("Limit 1/2 value monitoring of channel i :  0= not active, 1= Value is higher than    limit 1/2 value, 2= Value is lower than limit 1/2 value, 3: Value equals limit 1/2 value").arg("i","channel nr").arg("x","Limit nr");
    m_service->addOperation("Error",&SoemEL3062::is_error,this).doc("For the channel i : 1 = error (Overrange or Underrange ; 0 = no error ").arg("i","channel nr");
    m_service->addOperation("TxPDO",&SoemEL3062::Tx_PDO,this).doc("For the channel i :Valid of data of assigned TxPDO (0=valid, 1=invalid) ").arg("i","channel nr");
    m_service->addOperation("Toggle",&SoemEL3062::Tx_PDO_Toggle,this).doc("The TxPDO Toggle is toggled by the slave, if the data of the corresponding PDO was updated").arg("i","channel nr");
    ...
    m_service->addPort(m_raw_values_port).doc("AnalogMsg containing the read values of _all_ channels");
    ...
    for(unsigned int i=0;i<params.size();i++){
    	m_service->addProperty(this->getName()+params[i].name,params[i].param).doc(params[i].description);
    }
  ...
  bool SoemEL3062::configure(){
	  ...
	  for(unsigned int i=0;i<params.size();i++){
		  ...
		  temp_val=m_service->properties()->getProperty(this->getName()+params[i].name);
		  val=temp_val.get();

		  //assigning parameters
		  ec_SDOwrite (((m_datap->configadr)&0x0F),params[i].index,params[i].subindex,FALSE,params[i].size,&val,EC_TIMEOUTRXM);
	  }
  }
  ...
  void SoemEL3062::update(){
    ...
    if(m_values_port.connected()){
      ...
      m_msg.values[0]=(((out_el3062t*)(m_datap->inputs))->val_ch1*m_resolution);
      m_msg.values[1]=(((out_el3062t*)(m_datap->inputs))->val_ch2*m_resolution);
    }
  }
  ...
  int SoemEL3062::rawRead( unsigned int chan ){
    if(chan<m_size)
    {
    	if (chan==0) return ((out_el3062t*)(m_datap->inputs))->val_ch1;
    	if (chan==1) return ((out_el3062t*)(m_datap->inputs))->val_ch2;
    }
    return -1;
  }
  ...
  int SoemEL3062::read_param( unsigned int chan ){
    if(chan<m_size)
      {
	if (chan==0) return ((out_el3062t*)(m_datap->inputs))->param_ch1;
	if (chan==1) return ((out_el3062t*)(m_datap->inputs))->param_ch2;
      }
    return -1;
  }
  ...
  double SoemEL3062::read( unsigned int chan){
    ...
    if(chan<m_size)
      {
	if (chan==0) return (((out_el3062t*)(m_datap->inputs))->val_ch1*m_resolution);
	if (chan==1) return (((out_el3062t*)(m_datap->inputs))->val_ch2*m_resolution);
      }
    return -1;	
