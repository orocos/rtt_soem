/***************************************************************************
 tag: Ruben Smits  Tue Nov 16 09:31:20 CET 2010  soem_el4038.cpp

 soem_el4038.cpp -  description
 -------------------
...
SoemEL4038::SoemEL4038(ec_slavet* mem_loc) :
    soem_master::SoemDriver(mem_loc), size_(8),
                    ...
                    this->getName() + "_raw_values")
{
    ...
    service_->doc(std::string("Services for Beckhoff ") + std::string(
            datap_->name) + std::string(" Analog output module"));
    service_->addOperation("rawWrite", &SoemEL4038::rawWrite, this).doc(
            "Write raw value to channel i").arg("i", "channel nr").arg("value",
            "raw value");
    service_->addOperation("rawRead", &SoemEL4038::rawRead, this).doc(
            "Read raw value of channel i").arg("i", "channel nr");
    service_->addOperation("write", &SoemEL4038::write, this).doc(
            "Write value to channel i").arg("i", "channel nr").arg("value",
            "value");
    service_->addOperation("read", &SoemEL4038::read, this).doc(
    ...
    if (raw_values_port_.connected())
    {
        if (raw_values_port_.read(raw_msg_) == RTT::NewData)
            if (raw_msg_.values.size() <= size_)
                for (unsigned int i = 0; i < size_; i++)
                    ((out_el4038t*) (datap_->outputs))->values[i]
                            = raw_msg_.values[i];
    }
...
bool SoemEL4038::write(unsigned int chan, double value)
{
    if (chan < size_)
    {
        ...
        {
            ((out_el4038t*) (datap_->outputs))->values[chan] = (int) (value
                    / resolution_);
            return true;
        }
    }
