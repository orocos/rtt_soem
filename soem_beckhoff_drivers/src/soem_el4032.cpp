...
SoemEL4032::SoemEL4032(ec_slavet* mem_loc) :
    soem_master::SoemDriver(mem_loc), m_size(2), m_raw_range(65536), //32768
            m_lowest(-10.0), m_highest(10.0), m_values(m_size), m_raw_values(
                    m_size), m_values_port("values"), m_raw_values_port(
                    "raw_values")
{

    m_service->doc(std::string("Services for Beckhoff ") + std::string(
            m_datap->name) + std::string(" module"));
    m_service->addOperation("rawWrite", &SoemEL4032::rawWrite, this,
            RTT::OwnThread).doc("Write raw value to channel i").arg("i",
            "channel nr").arg("value", "raw value");
    m_service->addOperation("rawRead", &SoemEL4032::rawRead, this,
            RTT::OwnThread).doc("Read raw value of channel i").arg("i",
            "channel nr");
    m_service->addOperation("write", &SoemEL4032::write, this, RTT::OwnThread).doc(
            "Write value to channel i").arg("i", "channel nr").arg("value",
            "value");
    m_service->addOperation("read", &SoemEL4032::read, this, RTT::OwnThread).doc(
    ...
    if (m_raw_values_port.connected())
    {
        if (m_raw_values_port.read(m_raw_msg) == RTT::NewData)
        {
            if (m_raw_msg.values.size() <= m_size)
            {
                for (unsigned int i = 0; i < m_size; i++)
                {
                    ((out_el4032t*) (m_datap->outputs))->values[i]
                            = m_raw_msg.values[i];
                }
            }
        }
...
bool SoemEL4032::write(unsigned int chan, double value)
{
    if (chan < m_size)
    {
        ...
        {
            ((out_el4032t*) (m_datap->outputs))->values[chan] = (int) (value
                    / m_resolution);
            return true;
        }
    }
